...
class Roblox:
    ...
    def handle_multi(self, user_id_and_cookie) -> None:
        multiple_accounts = loads(user_id_and_cookie["errors"][0]["fieldData"]).get("users", [])
        usernames = [
            multiple_account.get("name")
            for multiple_account in multiple_accounts
            if multiple_account.get("name")
        ]
        account_line = f'{self.account[0]}:{self.account[1]}'
        if usernames:
            account_line = f'{account_line} | users: {", ".join(usernames)}'
        ...
        Output("ERROR").log(
            f"Multiple linked accounts | {self.account[0]}"
        )